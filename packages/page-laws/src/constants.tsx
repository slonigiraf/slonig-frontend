...
Keep learner-facing task text short and direct. Do not include solution steps, construction details that disclose answer-bearing information, or tutorial prose in the question unless carrying out that procedure is itself the target skill. Keep the solution concise; include only the result and the minimum work needed to verify the target operation. Templatability, self-containment, and an unambiguous learner operation take priority over the word target.`;
...
Choose abilityMode from the actual learner operation, with no globally preferred mode. Use "generation" when the learner must produce a response by decoding task-essential representational input into an answer, or when the learner must create or modify a representation as the answer. Use "transformation" when explicit symbolic or textual input is changed by a rule into another symbolic or textual result. Use "reasoning" when multi-step inference, justification, or deduction is the essential operation. Use the perceptual modes only when noticing or discriminating a perceptual property is itself the complete target rather than an intermediate step toward producing a represented value or constructed response. abilityMode must be one of: ${exerciseAbilityModes.join(', ')}.
...
Use <kx>...</kx> to surround every mathematical expression that uses KaTeX. Copy conceptIndex. Return only JSON in this shape: {"exercises":[{"conceptIndex":0,"title":"...","description":"succinct task","abilityMode":"...","solution":"concise correct solution","imageDescription":"","solutionImageDescription":""}]}. Replace abilityMode with exactly one allowed value.`;

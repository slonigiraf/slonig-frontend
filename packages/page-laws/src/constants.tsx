...
export const fixAbilitiesPrompt = 'Review every supplied Ability in this chapter and identify any error: factual, mathematical, logical, grammatical, spelling, ambiguity, incomplete or non-self-contained questions, incorrect or mismatched answers, mismatch between the two exercises and the target Skill, identical questions or concrete parameters, JSON/schema problems, and KaTeX syntax, escaping, or formula errors. The input contains every Ability from one chapter, each with its real stored id and chapter-local index; an Ability may be a parsed JSON object or a raw JSON string when its stored JSON is malformed. Also compare all supplied Abilities against one another and find duplicate Abilities within this chapter. A duplicate means two records represent the same Ability with the same narrow skill and the same or materially identical concrete question/answer content; do not call records duplicates merely because they train the same Skill with genuinely different concrete inputs. For every duplicate set, keep the earliest supplied index as the canonical record and return only the later duplicate record ids in duplicateAbilityIds. Never invent an id or index that is not present in the supplied chapter. Return reviews only for input indexes where you find an error. It is valid and preferred to omit correct Abilities entirely; a partial reviews array is expected. If there are no errors or duplicates return {"reviews":[],"duplicateAbilityIds":[]}. For an erroneous Ability, list concise error descriptions and include the complete corrected Ability in ability: {"index":0,"hasErrors":true,"errors":["description"],"ability":{...}}. If you choose to include a correct Ability, use {"index":0,"hasErrors":false,"errors":[]}, but this is unnecessary. Do not repair an Ability whose record id you place in duplicateAbilityIds because that record will be deleted. Correct every other identified error while preserving the same target Skill, language, method, and difficulty. Make the minimum changes necessary. Every corrected Ability must preserve the required i, t, h, and q structure, with t=3 and exactly two q entries. Do not change any existing i or p linkage values; the browser also preserves those fields locally. For malformed stored JSON, provide valid string values for i and p. The two exercises must train the same narrow skill but use different concrete inputs. Use <kx>...</kx> for every mathematical formula or expression, never dollar-delimited LaTeX, and escape every LaTeX backslash for valid JSON. Return only valid JSON in this exact top-level shape: {"reviews":[...],"duplicateAbilityIds":["stored-id-to-delete"]}. Do not add markdown fences or commentary.';
